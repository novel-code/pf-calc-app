...
const Sidebar = function ({mobileView}) {
  ...
  return (
    ...
    position: fixed; */}
     ...
     <div style={{width: "30px", zIndex: "999", position: "fixed", left: "20px", top: "15px"}} className="mobileViewDiv">
        <button style={{backgroundColor: "white"}} onClick={() => {
          setMobileBar(!mobileBar)
        }} >{mobileBar ? <GrClose></GrClose> :  <GrMenu></GrMenu>}</button>
        </div>
        {mobileBar ? 
        
          
        <div className="sidebar" style={{width: sidebar ? "80px" : "", transition: "0.3s"}}>
        <div style={{ alignSelf: "center", paddingTop: "15px"}} >
        <button className="sidebarButton" style={{backgroundColor: "white"}} onClick={() => {
          setSidebar(!sidebar)
        }}>{sidebar ? <GrMenu></GrMenu> : <GrClose></GrClose>}</button>
      ...
      <div className={styleSidebar.linkDiv}>
        <NavLink  style={{justifyContent: "center", display: "flex"}}
